...
namespace user
{
	namespace editor
	{
		...
		bool TerrainEditmodeColorMap::RaisePoint(glm::vec2 point, float Intensity)
		{
			...
			//how to mix colors?
			auto NewColor = TerrainEditGUI->GetSelectedColor();
		...
		void TerrainEditmodeColorMap::SaveTheFuckingMap()
		{
			...
			for (size_t t = 0; t < ColorMapSize - 1; t++)
			{
				for (size_t j = 0; j < ColorMapSize - 1; j++)
				{
			...
			uint8* buffer = new uint8[ColorMapSize * ColorMapSize * 8];
			Ogre::Image ogreImage;
			ogreImage.loadDynamicImage(buffer, ColorMapSize, ColorMapSize, Ogre::PixelFormat::PF_FLOAT32_GR);
			for (size_t t = 0; t < ColorMapSize - 1; t++)
			{
				for (size_t j = 0; j < ColorMapSize - 1; j++)
				{
