...
namespace user
{
	namespace editor
	{
		...
		*/
		class ImageDecompiler : public KC_AbstractView
		{
		...
		//[-------------------------------------------------------]
		public:
			static const uint32 PLUGINABLE_ID;	///< "user::editor::ImageDecompiler" unique pluginable view ID
		...
		public:
			...
			*/
			ImageDecompiler(qsf::editor::ViewManager* viewManager, QWidget* qWidgetParent);

			/**
			*  @brief
			*    Destructor
			*/
			virtual ~ImageDecompiler();
		...
		private:
			...
			Ui::ImageDecompiler*	mUiImageDecompiler;	///< UI view instance, can be a null pointer, we have to destroy the instance in case we no longer need it
...
//[ CAMP reflection system                                ]
//[-------------------------------------------------------]
QSF_CAMP_TYPE_NONCOPYABLE(user::editor::ImageDecompiler)
