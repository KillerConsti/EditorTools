...
namespace kc_terrain
{
	...
	//[ Public definitions                                    ]
	//[-------------------------------------------------------]
	const uint32 MoveCommand::PLUGINABLE_ID = qsf::StringHash("kc_terrain::MoveCommand");
	...
	bool MoveCommand::checkContext(const qsf::game::CommandContext& context)
	{
		...
		{
			QSF_LOG_PRINTS(INFO, "we have invalid pos")
		}
		em5::CommandContext::EvaluationReason evaluationReason = static_cast<const em5::CommandContext&>(context).mEvaluationReason;
		if (evaluationReason == em5::CommandContext::EVALUATIONREASON_SHOWCOMMAND_ABOVE_ENTITY)
		{
			// Don't show the icon in case we only show preview icons, it looks strange for this command
			return false;
		}
		QSF_LOG_PRINTS(INFO, "we have 0")
			if (context.mTargetEntity != nullptr)
			{
				if (context.mCaller == context.mTargetEntity)
					return false;
			}
		QSF_LOG_PRINTS(INFO, "we have 1")
		// We need a valid target position
		if (!context.mTargetPosition.is_initialized())
		{
			QSF_LOG_PRINTS(INFO, "we have invalid pos")
			if (nullptr != context.mTargetEntity)
				QSF_LOG_PRINTS(INFO, "we have targe ent" << context.mTargetEntity->getComponent<qsf::MetadataComponent>()->getName())
			glm::vec3 pos;
			if(!CheckNewTerrain(pos))
			return false;
			
			mpos = pos;
		}
		QSF_LOG_PRINTS(INFO, "we have 2")
...
#define ALLOW_CHAINORDER
		...
		moveTargetConfig.mPosition = *context.mTargetPosition;
			...
			moveTargetConfig.mPosition = mpos;
	...
	bool MoveCommand::CheckNewTerrain(glm::vec3& targetpos )
	{
		...
		qsf::RayMapQuery(QSF_MAINMAP).getFirstHitByRenderWindow(*EM5_APPLICATION.getRenderWindow(), (int)mousePosition.x, (int)mousePosition.y, response);
		if (response.component == nullptr)
		{
			...
			{
				auto vec = glm::vec3();
				if (TerrainMaster->getTerrainHitByRenderWindow(*EM5_APPLICATION.getRenderWindow(), (int)mousePosition.x, (int)mousePosition.y, &vec))
				{
					targetpos = vec;
					return true;
				}
	...
	bool MoveCommand::isTargetPositionBlockedByPolygons(qsf::Entity& caller, const glm::vec3& position)
	{
		...
		if (nullptr != routerComponent)
		{
			...
			if (entityHelper.isShip())
			{
				...
				//no api export
				//"em5::WaterDeepTypePolygonComponent"
				for (auto deepWaterPolygonComponent : qsf::ComponentMapQuery(QSF_MAINMAP).getAllInstancesById(qsf::StringHash("em5::WaterDeepTypePolygonComponent")))
				{
					auto Poly = static_cast<qsf::PolygonComponent*>(deepWaterPolygonComponent);

					Poly->isPositionInside(position);
					camp::UserObject UO = deepWaterPolygonComponent;
					auto Type = UO.get("WaterDeepType").to<std::string>();
					QSF_LOG_PRINTS(INFO, Type)
					if (Type == "DEEP_WATER" && Poly->isPositionInside(position))
					{
						isWaterDeep = true;
						break;
					}
